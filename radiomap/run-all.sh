...
# LOAD must be set to 1 if nodes are not yet running on R2lab
# so as to switch them on after having loaded load the right images
LOAD=0
...
# The "gathermap.py" nepi-ng script will do all the job and will create at the
# end a  directory for each scenario with all the ping logs and the pcap traces for each node
# 


for tx_power in $TX_POWER; do 
    for phy_rate in $PHY_RATE; do
	for ant_mask in $ANT_MASK; do
	    if test $LOAD -eq 1
	    then
		./gathermap.py -m "$NODES" -f "$CHANNEL" -r "$phy_rate" -a "$ant_mask" -T "$tx_power" -N "$PING_NB" -l
		LOAD=0
	    else
		./gathermap.py -m "$NODES" -f "$CHANNEL" -r "$phy_rate" -a "$ant_mask" -T "$tx_power" -N "$PING_NB"
	    fi
	done
    done
done
